...
def ChatGPT_API_with_finish_reason(model, prompt, response_format=None, api_key=None, chat_history=None):
    """
    ...
    Args:
        ...
        prompt: User prompt
        response_format: Optional response format (Pydantic BaseModel class for structured output)
        api_key: API key (optional, uses environment variables if not provided)
        chat_history: Previous conversation history

    Returns:
        Tuple of (response_content, finish_reason).
        If response_format is a Pydantic model, response_content is a parsed Pydantic instance.
    """
    max_retries = 10
    ...
    for i in range(max_retries):
        try:
            ...
            # If response_format is a Pydantic model, parse the JSON response
...
def ChatGPT_API(model, prompt, response_format=None, api_key=None, chat_history=None):
    ...
    Returns:
        Response content string, or parsed Pydantic instance if response_format is provided.
    """
    max_retries = 10
